"""
실제 센서 데이터 파싱 모듈
CSV 형태의 센서 데이터를 파싱하여 데이터베이스에 저장
"""
import re
...
class SensorDataParser:
    ...
    def __init__(self):
        self.line_mapping = {
            'L0-BREAST': {'id': 'L0', 'side': 'PORT', 'type': 'BREAST', 'index': 0},
            'L1-BREAST': {'id': 'L1', 'side': 'STARBOARD', 'type': 'BREAST', 'index': 0},
            'L2-SPRING': {'id': 'L2', 'side': 'PORT', 'type': 'SPRING', 'index': 1},
            'L3-SPRING': {'id': 'L3', 'side': 'STARBOARD', 'type': 'SPRING', 'index': 1},
            # 추가 4개 계류줄 (확장용)
            'L4-BREAST': {'id': 'L4', 'side': 'PORT', 'type': 'BREAST', 'index': 2},
            'L5-BREAST': {'id': 'L5', 'side': 'STARBOARD', 'type': 'BREAST', 'index': 2},
            'L6-SPRING': {'id': 'L6', 'side': 'PORT', 'type': 'SPRING', 'index': 3},
            'L7-SPRING': {'id': 'L7', 'side': 'STARBOARD', 'type': 'SPRING', 'index': 3}
        }
    ...
    def parse_csv_line(self, line: str) -> Optional[Dict]:
        ...
        반환:
        {
            ...
            'distance': 14.5,
            'lines': {
                'L0-BREAST': {'tension': 0.95, 'length': 0.000},
                'L1-BREAST': {'tension': 1.55, 'length': 0.000},
                ...
            }
        }
        """
        try:
            ...
            while i < len(parts):
                if i + 5 < len(parts):
                    ...
                    if line_name in self.line_mapping:
    ...
    def save_parsed_data(self, parsed_data: Dict, db: Session):
        ...
        try:
            for line_name, line_data in parsed_data['lines'].items():
                ...
                line_info = self.line_mapping[line_name]
                
                # 계류줄 정보 조회 또는 생성
                mooring_line = db.query(MooringLine).filter(
                    MooringLine.line_id == line_info['id']
                ).first()
