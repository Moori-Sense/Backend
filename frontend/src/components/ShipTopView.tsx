...
const ShipTopView: React.FC<ShipTopViewProps> = ({ 
  ...
  return (
    <div className="bg-white rounded-xl shadow-lg p-6">
      <div className="relative mx-auto" style={{ width: '500px', height: '700px' }}>
        {/* 배 본체 - 더 길쭉하게 */}
        <div className="absolute inset-0">
          ...
          <div 
            ...
            style={{
              ...
              width: '350px',
              height: '600px',  // 더 길게
              borderRadius: '175px 175px 20px 20px'
            }}
          >
            {/* 선교 (브릿지) - 중앙에 배치 */}
            <div 
        ...
        <div className="absolute left-0 top-1/2 transform -translate-y-1/2">
          <div className="space-y-12">
            {portLines.slice(0, 4).map((line, index) => (
              <div key={line.id} className="flex items-center">
                <MooringLineButton 
                  ...
                  index={index} 
                />
                <div className="ml-2 w-20 h-1 bg-gray-700 rounded"></div>
              </div>
            ))}
          </div>
        </div>

        {/* 우현 (STARBOARD) 계류줄들 - 4개 */}
        <div className="absolute right-0 top-1/2 transform -translate-y-1/2">
          <div className="space-y-12">
            {starboardLines.slice(0, 4).map((line, index) => (
              <div key={line.id} className="flex items-center">
                <div className="mr-2 w-20 h-1 bg-gray-700 rounded"></div>
                <MooringLineButton 
                  line={line} 
                  position="starboard" 
                  index={index} 
